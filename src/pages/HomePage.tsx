...
const HomePage: React.FC = () => {
  ...
  // 강사진 데이터 변환
  const instructorItems = instructors.length > 0
    ...
    : [
        { title: '김민수 강사', description: '한국예술종합학교 출신', link: '/instructor/1', imageUrl: '' },
        { title: '이지은 강사', description: '중앙대학교 연극학과 출신', link: '/instructor/2', imageUrl: '' },
      ];

  // 인스타그램 데이터 변환
  const instagramItems = instagramPosts.length > 0
  ...
  // 유튜브 데이터 변환
  const youtubeItems = youtubeVideos.length > 0
    ...
    : [
        { title: '합격생 인터뷰 1', description: '2024 합격생의 솔직한 이야기', link: '/video/1', thumbnailUrl: '' },
        { title: '합격생 인터뷰 2', description: '민액터스 수강 후기', link: '/video/2', thumbnailUrl: '' },
      ];
