...
const PassersListPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPassers = async (): Promise<void> => {
      setLoading(true);
      try {
        const response = await passerAPI.getAll({
          page: currentPage,
          limit: 10,
        });
        setPassers(response.data.data);
  ...
  const seoData = PAGE_SEO['/passers'];
  ...
  return (
    <PageContainer>
      ...
      />
      <JsonLdScript data={createBreadcrumbSchema(breadcrumbConfig['/passers'])} />
      <PageTitle>실시간 합격자 현황</PageTitle>
          ...
          {passers.map(passer => (
            <ListItem key={passer._id} onClick={() => handleRowClick(passer._id)}>
              <ListItemInfo>
                ...
                <ItemTitle>{passer.title}</ItemTitle>
              </ListItemInfo>
              <ItemMeta>
                <span>최고관리자</span>
...
export default PassersListPage;
